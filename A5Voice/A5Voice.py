...
# find . -name "*.raw" -exec bash -c "sox -r 48k -e signed -b 32 -c 1 --endian little {} {}.wav" \;
...
#---------------------------------------------
# Crude decoding of Preset
# Requires that the binary file covers this memory location

'''
Preset table starts after a big section of '00's
015ffff0  00 00 00 00 00 00 00 00  00 00 00 00 e0 34 06 00  |.............4..|
01600000  00 00 00 00 00 00 00 00  00 00 00 00 00 00 00 00  |................|
*
02000000  6a 00 00 00 0d 53 61 77  70 72 65 73 73 69 76 65  |j....Sawpressive|
02000010  20 47 44 00 20 00 20 00  b0 04 00 00 04 00 20 00  | GD. . ....... .|

Maybe the info from Edisyn would hint at encoding?
...
PRESETS = Struct(
        "presets" / Array(5, Array(128, PRESET)),  # seems to support 5 banks of 128, although space for 10 banks
)
...
def main():
    ...
    parser.add_option("-P", "--preset",
        action="store_true", dest="preset",
        help="list the presets in file (if available)")
    ...
    if data:
        ...
        # Requires that the binary file covers this memory location
        if options.preset and len(data) > 0x2000000:
            presets = PRESETS.parse(data[0x2000000:])

            print()
            for group in presets['presets']:
                for preset in group:
